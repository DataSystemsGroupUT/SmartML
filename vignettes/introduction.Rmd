...
#' Option 1 = Classifier Selection Only, apply PCA as a preprocessing step with 4 components, get two candidate models as output only, and impute missing values using mice
result1 <- autoRLearn(1, system.file("extdata", "shuttle/train.arff", package = "SmartML"), system.file("extdata", "shuttle/train.arff", package = "SmartML"), option = 1, preProcessF = 'pca', nComp = 4, nModels = 2, missingOpr = TRUE) #option 1 runs for Classifier Algorithm Selection Only
```
```{r, warning = FALSE}
...
result2 <- autoRLearn(2, system.file("extdata", "shuttle/train.arff", package = "SmartML"), system.file("extdata", "shuttle/test.arff", package = "SmartML"), interp = TRUE) # Option 2 runs for both classifier algorithm selection and parameter tuning for 2 minutes.
```
```{r, warning = FALSE}
...
```{r, warning = FALSE, results = 'hide'}
#' Option 2 = Both Classifier Selection and Parameter Optimization, use 20% validation set from training set, select only features (1,3,5,7,9,11,13,15) from dataset, select only 2 classifiers to be tuned and apply MICE for missing values imputation
result3 <- autoRLearn(3, system.file("extdata", "car/train.arff", package = "SmartML"), system.file("extdata", "car/test.arff", package = "SmartML"), vRatio = 0.2, selectedFeats = c(1,2,5,6,7), nModels = 2) # Option 2 runs for both classifier algorithm selection and parameter tuning for 3 minute.
```
```{r, warning = FALSE}
